    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if author not in player.voice_client.channel.voice_members and author_perms.skip_when_absent:
                ...
                newmsg = 'I am Groot! Now playing in `%s`: `%s` added by `%s`' % (
                    player.voice_client.channel.name, entry.title, entry.meta['author'].name)
...
#######################################################################################################################
    ...
    def _gen_embed(self):
        ...
        e = discord.Embed()
        e.colour = 7506394
        e.set_footer(text='MusicBot ({})'.format(BOTVERSION), icon_url='https://i.imgur.com/gFHBoZA.png')
        return e
            ...
            if self.config._spotify:
                ...
                try:
                    ...
                    if song_url.startswith('track:'):
                        ...
                        await self._do_playlist_checks(permissions, player, author, res['tracks']['items'])
                        procmesg = await self.safe_send_message(channel, self.str.get('cmd-play-spotify-playlist-process', 'I am Grooting! Processing playlist `{0}`').format(res['name']))
                        for i in res['tracks']['items']:
